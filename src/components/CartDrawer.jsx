...
const CartDrawer = ({ isOpen, onClose }) => {
  ...
  // Don't show the cart drawer if checkout is open
  if (!portalRoot) return null;

  const showDrawer = isOpen && !isCheckoutOpen;
          ...
          {cart.length === 0 ? (
            ...
            <>
              {cart.map((item) => (
                <div key={item.menuItem.id} className="mb-4 flex gap-3 rounded-lg border border-border-color/70 p-3 dark:border-slate-700">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-text-dark dark:text-white">{item.menuItem.name}</div>
              ...
              <div className="mt-4 border-t border-border-color/80 pt-4 dark:border-slate-800">
                <div className="flex justify-between text-lg font-bold text-text-dark dark:text-white">
                  <span>Total:</span>
